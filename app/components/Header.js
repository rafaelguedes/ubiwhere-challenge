...
class Header extends React.Component {
    ...
    componentDidMount() {
        ...
        if(storeState) {
            this.setState(() => ({ username: storeState.username, email: storeState.email, userLogged: storeState.userLogged }));
        }
    }
    ...
    isRegistered() {
        ...
        var userMatch = this.state.users.find(user => user.username === this.state.username && user.email === this.state.email);
        
        if(userMatch) {
            this.setState(() => ({ userLogged: userMatch.id }));
            //Callback to tell parent component that the user is logged in + his ID
            this.props.callback(this.state.userLogged);
        } else {
            this.registerUser();
        }
                        ...
                        ?   <div className='header__form col-12 col-md-9'>
                                <form className='header__form-group form-inline input-group-lg'>
                                    <input type='text' 
                                        ...
                                        style={{display: this.state.userLogged ? 'none' : 'block' }}>
                                    </input>
                                    <input type="text" 
                                        className="header__form-group-input form-control" 
                                    ...
                                    <p className="header__form-greet paragraph paragraph--bold" 
                                        style={{display: this.state.userLogged ? 'block' : 'none' }}>
                                        Hi, {this.state.username}!
                                    </p>
                                    <button type="submit" 
                                        className="header__form-group-btn btn-lg btn-primary btn-login" 
                                        onClick={this.handleClick}>
                                        {this.state.userLogged ? 'Log Out' : 'Log In'}
                                    </button>
                                </form>
                            </div> 
                        : null}
